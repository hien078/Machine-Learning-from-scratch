...
| Symbol | Type | Meaning |
|--------|------|---------|
| $\mathcal{D}$ | Set | Unlabeled dataset $\{x_1, x_2, \dots, x_N\}$ |
...
2. **Base Encoder ($f_\theta$):** Extracts representation vectors $h_i = f_\theta(\tilde{x}_i)$ and $h_j = f_\theta(\tilde{x}_j)$.
3. **Projection Head ($g_\phi$):** Maps $h$ to a lower-dimensional embedding space $z_i = g_\phi(h_i)$, normalized to the unit hypersphere $\|z_i\|_2 = 1$.
...
InfoNCE generalizes NCE to a multi-class classification problem. Given an anchor sample $x$ and its positive target $y^+ \sim p(y|x)$, along with $K$ independent noise samples $\{y_1, y_2, \dots, y_K\} \sim p(y)$, the model must identify $y^+$ out of the set of $K+1$ candidates.
...
1. **Optimal Classifier Probability:**
   By Bayes' Rule, the true posterior probability that candidate $i$ is the positive sample given candidates $V = \{y_1, \dots, y_{K+1}\}$ is:
...
**Result:** $\displaystyle \mathcal{L}_{\text{Barlow}} = \sum_i (1 - \mathcal{C}_{ii})^2 + \lambda \sum_i \sum_{j \neq i} \mathcal{C}_{ij}^2$
